      ...
      {pipelinePhases.map((_, i) => {
        ...
        return (
          <div key={i} className="relative flex items-center">
            {inRange && (
              <div className="absolute inset-y-1 inset-x-0.5 rounded bg-navy-700/50 flex items-center justify-center">
                {label && i === startCol && (
                  <span className="text-[10px] font-medium text-navy-300 tracking-wide">{label}</span>
                )}
              </div>
            )}
          </div>
                  ...
                  {pipelinePhases.map((_, ci) => {
                    ...
                    return (
                      <td key={ci} className="px-1 py-2">
                        {inRange && (
                          <div className="h-7 rounded bg-navy-700/50 flex items-center justify-center">
                            {row.label && ci === row.startCol && (
                              <span className="text-[10px] font-medium text-navy-300 tracking-wide">{row.label}</span>
                            )}
                          </div>
                        )}
                      </td>
            ...
            {group.rows.map((row, ri) => (
              <div key={ri} className="px-4 py-3 border-b border-navy-700/30 last:border-0">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-navy-300 bg-navy-800/50 px-2.5 py-1 rounded-full">
